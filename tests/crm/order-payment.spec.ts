import { TEST_PACKAGES, generateStudent } from "../../src/data/factories";
import { TEXT } from "../../src/data/text";
import { createDealWithNewParent } from "../../src/flows/deal.flow";
import { expect, test } from "../../src/fixtures/test";

test.describe("CRM — Đơn hàng & thanh toán", () => {
  for (const pkg of TEST_PACKAGES) {
    test(`Thanh toán đủ đơn "${pkg.name}" → deal lên L8.2 và sinh deal OB Upsell`, async ({
      dealListPage,
      newDealDialog,
      dealDetailDialog,
      orderCreateDialog,
      ...
      const parent = await createDealWithNewParent(dealListPage, newDealDialog);
      ...
      // 4. Chọn gói sản phẩm, nhập email PH, phương thức hoá đơn COD
      await orderCreateDialog.selectProduct(pkg.code);
